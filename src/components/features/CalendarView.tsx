...
export function CalendarView() {
  ...
  useEffect(() => {
    ...
    const linkEvents = () => {
      googleEvents.forEach((gEvent) => {
        const gDate = gEvent.start.dateTime
          ? new Date(gEvent.start.dateTime)
          : gEvent.start.date
            ? parseISO(gEvent.start.date)
            : null;
        if (!gDate) return;
  ...
  const getDayEvents = (date: Date) => {
    ...
    const regularTransactions = transactions.filter((t) => !t.isRecurring && isSameDay(new Date(t.date), date));
    ...
    const recurringTransactions = transactions
      .filter((t) => {
        ...
        if (isDayMatch && isAfterStart) {
          ...
          const hasRealization = regularTransactions.some(
            (realTx) =>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full min-h-0">
      <AddTransactionModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} defaultDate={selectedDate} />
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  !isCurrentMonth
                    ? 'opacity-30 bg-muted/10 border-transparent text-muted-foreground'
                    : 'bg-card/50 border-border/50 text-foreground',
                  isSelected ? 'ring-2 ring-primary border-transparent bg-primary/10 z-10' : 'hover:bg-muted/50',
              ...
              >
                <div className="flex justify-between w-full px-1">
                  ...
                  <div className="flex gap-0.5">
                    {hasGoogleEvent && (
                      <div className="h-1.5 w-1.5 rounded-full bg-violet-500" title="Evento de Google" />
                    )}
                  </div>
                </div>
                ...
                <div className="flex gap-1 mt-auto mb-1">
                  {hasIncome && (
                    <div className="h-1 lg:h-1.5 w-3 lg:w-4 rounded-full bg-emerald-500/80 shadow-[0_0_8px_rgba(16,185,129,0.5)]" />
                  )}
                  {hasExpense && (
                    <div className="h-1 lg:h-1.5 w-3 lg:w-4 rounded-full bg-red-500/80 shadow-[0_0_8px_rgba(239,68,68,0.5)]" />
                  )}
                  {hasSaving && (
                    <div className="h-1 lg:h-1.5 w-3 lg:w-4 rounded-full bg-blue-500/80 shadow-[0_0_8px_rgba(59,130,246,0.5)]" />
                  )}
                </div>
              </div>
            );
          ...
          <>
            ...
            <div className="p-4 flex-1 overflow-y-auto space-y-3 custom-scrollbar">
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        <p
                          className={cn(
                            'font-bold text-sm truncate text-foreground',
                            showAsCompleted && 'line-through opacity-70',
                          )}
                        >
                          {t.description}
                        </p>
                      ...
                      <p className="text-xs font-medium opacity-70 flex items-center gap-1">
                        {t.isRecurring && (
                          <span className="text-[10px] bg-foreground/10 px-1.5 py-0.5 rounded-full">Repetir</span>
                        )}
                        ...
                        {t.isSyncedWithGoogle && // Note: CalendarView logic sets 'isSyncedWithGoogle' if t.google_event_id is present or matched.
                          (t.google_event_id === 'PENDING_SYNC' ? (
                            <span
                              className="text-[10px] bg-amber-500/10 text-amber-600 dark:text-amber-500 px-1.5 py-0.5 rounded-full border border-amber-500/20 flex items-center gap-0.5"
                            ...
                            <span
                              className="text-[10px] bg-background/50 text-foreground px-1.5 py-0.5 rounded-full border border-border/50 shadow-sm flex items-center gap-1"
                        ...
                        {showAsCompleted && (
                          <span className="text-[10px] text-green-600 font-bold ml-1">
              ...
              {getDayEvents(selectedDate).googleEvents.map((e) => (
                <div
                  key={e.id}
                  className="flex items-center gap-3 p-3 rounded-xl bg-violet-50 dark:bg-violet-500/10 border border-violet-100 dark:border-violet-500/20 hover:scale-[1.01] transition-all"
                >
              ...
              {getDayEvents(selectedDate).transactions.length === 0 &&
                getDayEvents(selectedDate).googleEvents.length === 0 && (
