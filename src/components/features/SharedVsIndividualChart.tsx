...
export function SharedVsIndividualChart({ className }: { className?: string }) {
  ...
  const data = useMemo(() => {
    ...
    const individualTotal = expenses.filter((t) => t.isShared === false).reduce((sum, t) => sum + t.amount, 0);

    const sharedTotal = expenses.filter((t) => t.isShared === true).reduce((sum, t) => sum + t.amount, 0);
      ...
      <div className="flex-1 w-full min-h-0 flex items-center justify-center">
        <ResponsiveContainer width="100%" height="100%">
          <PieChart>
            ...
            />
            <Legend verticalAlign="bottom" height={36} content={(props) => <CustomLegend {...props} />} />
          </PieChart>
        </ResponsiveContainer>
      </div>
    </GlassCard>
