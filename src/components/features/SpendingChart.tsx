...
export function SpendingChart({ className }: { className?: string }) {
  ...
  const data = useMemo(() => {
    ...
    return days.map((day) => {
      ...
      if (viewMode === 'type') {
        // Breakdown by Type
        item['Ingresos'] = dayTransactions.filter((t) => t.type === 'income').reduce((acc, t) => acc + t.amount, 0);
        item['Gastos'] = dayTransactions.filter((t) => t.type === 'expense').reduce((acc, t) => acc + t.amount, 0);
        item['Ahorros'] = dayTransactions.filter((t) => t.type === 'saving').reduce((acc, t) => acc + t.amount, 0);
      } else {
