      ...
      <div className={cn('space-y-3 overflow-y-auto pr-2 flex-1 min-h-0', !className && 'max-h-125 lg:max-h-none')}>
        {transactions.map((t) => {
          ...
          return (
            <GlassCard
              key={t.id}
              className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 gap-3 sm:gap-0 group"
            >
              <div className="flex items-center gap-4 flex-1 min-w-0 w-full sm:w-auto">
                <div
                  className={cn(
                    'flex h-10 w-10 items-center justify-center rounded-xl text-lg shrink-0 shadow-lg text-white',
                    catColor,
                  )}
                >
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p
                      className={cn(
                        'font-medium text-foreground truncate max-w-50 sm:max-w-none',
                        t.status === 'paid' && 'line-through opacity-70',
                      )}
                    >
                      {t.description}
                    </p>
                  ...
                  <div className="flex items-center flex-wrap gap-2 text-xs text-muted-foreground mt-0.5">
                    ...
                    {t.isShared === false ? (
                      <span
                        className="flex items-center gap-1 text-[10px] bg-blue-500/10 text-blue-500 px-1.5 py-0.5 rounded-md border border-blue-500/20 shrink-0"
                        title="Visible solo para ti"
                      >
                      ...
                      <span
                        className="flex items-center gap-1 text-[10px] bg-purple-500/10 text-purple-500 px-1.5 py-0.5 rounded-md border border-purple-500/20 shrink-0"
                        title="Compartido con el grupo"
                      >
                    ...
                    {t.google_event_id &&
                      (t.google_event_id === 'PENDING_SYNC' ? (
                        <span
                          className="flex items-center gap-1.5 text-[10px] bg-amber-500/10 text-amber-600 dark:text-amber-500 px-2 py-1 rounded-full border border-amber-500/20 shadow-sm shrink-0"
                          title="Pendiente de sincronización (Esperando conexión)"
                        >
                          <Clock className="h-3 w-3" />
                        ...
                        <span
                          className="flex items-center gap-1.5 text-[10px] bg-background/50 text-foreground px-2 py-1 rounded-full border border-border/50 shadow-sm shrink-0"
              ...
              <div className="flex items-center justify-between w-full sm:w-auto sm:justify-end gap-4 mt-2 sm:mt-0 ml-0 sm:ml-4 border-t sm:border-t-0 border-border/10 pt-2 sm:pt-0">
                <div className="text-right">
                  <p
                    className={cn(
                      'font-bold',
                      t.type === 'income' ? 'text-emerald-500' : t.type === 'saving' ? 'text-blue-500' : 'text-red-500',
